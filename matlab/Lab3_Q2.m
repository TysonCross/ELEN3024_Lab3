...
% minus since we choose upper sideband
...
demodulatedIP=filter(LPF,firstStageDemodulatedIP);

demodulatedIP = (demodulatedIP); %% remove offset, is (A/a)-1/a

LPF_stage=fdesign.lowpass('N,F3dB', filterOrder, cutoffFreq, 20*f_m);
LPF=design(LPF_stage);
demodulatedQP=filter(LPF,firstStageDemodulatedQP);

demodulatedQP = (demodulatedQP);  %% remove offset, is (A/a)-1/a
...
demodulated_frequency =(A_m/N)  * abs(fftshift((fft(demodulatedIP))));
x_lim_4 = [-f_m f_m]*1.5;
ax2=subplot(2,2,3);
